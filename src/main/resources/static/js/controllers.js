...
function invoiceRefundSellCtrl($scope, $rootScope, $http, DataServiceApi, storageService, validateForms, toaster, toastrService, $stateParams, localStorageService, $interval, SweetAlert, $translate) {
    ...
    $scope.addInvoice = function (form) {
        ...
        if (form.validate()) { // Check inputs validation
            ...
            // Save invoice
            DataServiceApi.PostData($scope.invoice, server + "invoice/refundSell/add").then(function (res) {
                if (res.status === 200 && res.data > 0) {
                    toastrService.success('Success', 'Invoice saved successfully');
                    console.log(res.data);
                } else {
    ...
    function saveRefundSellInvoiceToLocalStorage() {
        localStorageService.set("refundSellTaxes", $scope.ckb);
        localStorageService.set("refundSellDiscount1", $scope.dis1ckb);
        localStorageService.set("refundSellDiscount2", $scope.dis2ckb);
        $scope.invoice.invoiceItemsList = $scope.invoiceItems
        localStorageService.set("refundSellInvoice", $scope.invoice);
    ...
    function getRefundSellInvoiceFromLocalStorage() {
        ...
        $scope.invoice = localStorageService.get("refundSellInvoice") //get object from local storage
        angular.forEach($scope.invoice.invoiceItemsList, function (val) {
            $scope.invoiceItems.push(val);
        });
        $scope.ckb = localStorageService.get("refundSellTaxes");
        $scope.dis1ckb = localStorageService.get("refundSellDiscount1");
        $scope.dis2ckb = localStorageService.get("refundSellDiscount2");
